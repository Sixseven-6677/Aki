...
# ws3-fca is intentionally absent from package.json to avoid npm resolving it
# via SSH during this step — it is installed separately in Step 3.
...
# Step 3: Install ws3-fca via HTTPS tarball (no SSH/git-credentials needed)
# ws3-fca by ntkhang03 — actively maintained, replaces @dongdev/fca-unofficial
RUN npm install --legacy-peer-deps --no-save --no-package-lock \
