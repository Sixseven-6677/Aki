...
const fcaHelpers_1 = require("../../../facebook/fcaHelpers");
// ─── Helpers ────────────────────────────────────────────────────────────────
const HEADER = "⸪⟅𝐕̶݈̂͜𝔈̟͢⃟݃།̶𝝬̶۪͛ۡ⸸𝚬̱̩⩨ܵ𝐁᮫͎ܺ݀ࣸ᷼᷍⃢ː𝚶̶݄݈݊𝐓݂ ❈ 🦢";
function getApi(pluginCtx) {
    const primary = pluginCtx.consumeService("mirai-transport")?.getApi?.() ?? null;
    if (primary)
        return primary;
    return pluginCtx.consumeService("mirai-transport-secondary")?.getApi?.() ?? null;
}
/** Resolve a list-position (1-based) or raw FB ID from a string arg. */
function resolveTarget(arg, adminIDs) {
    if (!arg)
        return null;
...
// ─── Sub-handlers ────────────────────────────────────────────────────────────
async function handleListAdmins(ctx, pluginCtx) {
    await ctx.typingOn();
    ...
    let info;
    try {
        info = await (0, fcaHelpers_1.fetchThreadInfo)(api, ctx.thread.id);
    }
    catch (err) {
...
async function handleAddAdmin(ctx, pluginCtx) {
    await ctx.typingOn();
    const api = getApi(pluginCtx);
    if (!api) {
        await ctx.reply("⚠️ خدمة Facebook غير متاحة حالياً.");
    ...
    try {
        await (0, fcaHelpers_1.setAdminStatus)(api, ctx.thread.id, [targetId], true);
        const name = info.userInfo[targetId]?.name ?? targetId;
...
async function handleRemoveAdmin(ctx, pluginCtx) {
    await ctx.typingOn();
    const api = getApi(pluginCtx);
    if (!api) {
        await ctx.reply("⚠️ خدمة Facebook غير متاحة حالياً.");
    ...
    if (!targetId) {
        await ctx.reply("⚠️ الرجاء تحديد رقم الأدمن من القائمة أو معرّفه.\n" +
            " مثال: ادمن حذف 2\n" +
    ...
    try {
        await (0, fcaHelpers_1.setAdminStatus)(api, ctx.thread.id, [targetId], false);
        const name = info.userInfo[targetId]?.name ?? targetId;
...
class AdminPlugin {
    ...
    async onEnable() {
        ...
        pluginCtx.logger.info(`Command "${adminCommand.name}" registered ` +
...
exports.default = new AdminPlugin();
