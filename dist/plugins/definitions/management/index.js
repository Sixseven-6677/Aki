...
const ProtectionRegistry_1 = require("../../../protection/ProtectionRegistry");
 ...
 */
async function loadFromMongoWithRetry(repo, log, maxAttempts = 3, retryDelayMs = 2_000) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            const docs = await repo.findAll();
            const store = { threads: {}, botNicknames: {} };
            for (const d of docs) {
            ...
            if (attempt < maxAttempts) {
                log.warn(`ManagementPlugin: MongoDB load failed (attempt ${attempt}/${maxAttempts}) — retrying in ${retryDelayMs}ms.`, { error: msg });
                await new Promise((r) => setTimeout(r, retryDelayMs));
            }
            else {
                log.warn(`ManagementPlugin: MongoDB load failed after ${maxAttempts} attempts — falling back to file.`, { error: msg });
            }
        }
    }
    return null;
...
function getThreadState(data, threadID) {
...
function saveThreadState(store, threadId, repo, logger) {
    if (!repo)
        return;
...
function fcaGetThreadInfo(api, threadID) {
...
function fcaSetTitle(api, title, threadID) {
    return new Promise((resolve, reject) => {
...
function fcaChangeNickname(api, nick, threadID, userID) {
    return new Promise((resolve, reject) => {
...
function getApi(pCtx) {
    const primary = pCtx.consumeService("mirai-transport")?.getApi?.() ?? null;
    if (primary)
        return primary;
    return pCtx.consumeService("mirai-transport-secondary")?.getApi?.() ?? null;
}
async function assertGroupAdmin(ctx, api, pCtx) {
    let info;
    try {
 ...
 */
async function handleGroupName(ctx, pCtx, store, repo) {
    await ctx.typingOn();
    ...
    (0, ProtectionRegistry_1.setProtectionStore)(store);
    try {
        await fcaSetTitle(api, newName, ctx.thread.id);
        saveThreadState(store, ctx.thread.id, repo, pCtx.logger);
        (0, group_handler_1.logProtectionEnabled)(ctx.thread.id, newName, ctx.user.id);
...
async function handleBotName(ctx, pCtx, store, repo) {
    await ctx.typingOn();
...
async function handleSetNickname(ctx, pCtx, store, repo) {
    await ctx.typingOn();
    const api = getApi(pCtx);
    if (!api) {
        await ctx.reply("⚠️ خدمة Facebook غير متاحة.");
    ...
    if (failed > 0)
        lines.push(`⌯ فشل: ${failed} عضو`);
    if (botNick && nickBotRestoreOk)
        lines.push(`⌯ اسم البوت محمي: "${botNick}"`);
    if (botNick && !nickBotRestoreOk)
        lines.push(`⚠️ تعذّر استعادة اسم البوت — أعد تعيينه: بوت ${botNick}`);
    await ctx.reply(lines.join("\n"));
}
// ─── Protection sub-handlers ──────────────────────────────────────────────────
async function handleProtection(ctx, pCtx, store, repo) {
 ...
 */
async function handleProtectNameEnable(ctx, pCtx, store, repo) {
    const api = getApi(pCtx);
    if (!api) {
        await ctx.reply("⚠️ خدمة Facebook غير متاحة.");
        return;
    ...
    saveThreadState(store, ctx.thread.id, repo, pCtx.logger);
    (0, group_handler_1.logProtectionEnabled)(ctx.thread.id, threadState.lockedName, ctx.user.id);
 ...
 */
async function handleProtectNameDisable(ctx, pCtx, store, repo) {
    const api = getApi(pCtx);
    if (!api) {
        await ctx.reply("⚠️ خدمة Facebook غير متاحة.");
        return;
    ...
    threadState.protectName = false;
    saveThreadState(store, ctx.thread.id, repo, pCtx.logger);
    (0, group_handler_1.logProtectionDisabled)(ctx.thread.id, ctx.user.id);
 ...
 */
async function handleProtectNameToggle(ctx, pCtx, store, repo) {
    const api = getApi(pCtx);
    if (!api) {
        await ctx.reply("⚠️ خدمة Facebook غير متاحة.");
        return;
    ...
    if (!threadState.protectName) {
        ...
        threadState.lockedName = info.name;
        saveThreadState(store, ctx.thread.id, repo, pCtx.logger);
        (0, group_handler_1.logProtectionEnabled)(ctx.thread.id, info.name, ctx.user.id);
...
async function handleProtectNicknames(ctx, pCtx, store, repo) {
    const api = getApi(pCtx);
    if (!api) {
        await ctx.reply("⚠️ خدمة Facebook غير متاحة.");
        return;
...
async function handleClearNicknames(ctx, pCtx, store, repo) {
    await ctx.typingOn();
    const target = ctx.getArg(0);
    if (target !== "كنيات") {
...
class ManagementPlugin {
    ...
    async onLoad(ctx) {
        this.ctx = ctx;
        this.repo = ctx.consumeService("group-settings-repo") ?? null;
        if (this.repo) {
            ...
            if (loaded) {
                ...
                ctx.logger.warn("ManagementPlugin: MongoDB unavailable after retries — starting with empty state.", { savedThreads: 0 });
            }
        }
        else {
            this.store = { threads: {}, botNicknames: {} };
            ctx.logger.warn("ManagementPlugin: no MongoDB repo — starting with empty protection state.", { savedThreads: 0 });
        }
    }
    ...
    async onEnable() {
        ...
        pCtx.logger.info("ManagementPlugin v8 enabled. [watchdog-start] " +
    ...
    startWatchdog(pCtx) {
        ...
        this.watchdogTimer = setInterval(() => {
            this.watchdogTick(pCtx).catch((err) => {
    ...
    async watchdogTick(pCtx) {
        ...
        if (!this.lastConnectedState && isConnected) {
            pCtx.logger.info("ManagementPlugin: transport just reconnected — reloading protection state from MongoDB. [self-healing]");
            await this.reloadFromMongo(pCtx);
        }
        // ── Warn if transport is down ─────────────────────────────────────────
        if (!isConnected) {
            pCtx.logger.warn("ManagementPlugin: transport is DISCONNECTED — name protection INACTIVE. [protection-warning]", {
                protectedNames: summary.protectedNames,
    ...
    async reloadFromMongo(pCtx) {
        ...
        if (loaded) {
            this.store = loaded;
            (0, ProtectionRegistry_1.setProtectionStore)(this.store);
            const summary = (0, ProtectionRegistry_1.getProtectionSummary)();
            pCtx.logger.info("ManagementPlugin: protection state reloaded from MongoDB after reconnect. [protection-reloaded]", {
                protectedNames: summary.protectedNames,
                totalThreads: summary.totalThreads,
                timestamp: new Date().toISOString(),
            });
...
exports.default = new ManagementPlugin();
