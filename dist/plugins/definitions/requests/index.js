...
function fetchTag(api, tag) {
    return new Promise((resolve) => {
        const timer = setTimeout(() => resolve({ tag, list: [], err: "timeout" }), 14_000);
        try {
 ...
 */
async function fetchAllPending(api, pCtx) {
    ...
    for (const t of inboxThreads) {
        ...
        if (!folderLabel || folderLabel === "INBOX")
            continue; // skip normal inbox
        seen.add(t.threadID);
        entries.push({ threadID: t.threadID, name: threadName(t), folder: folderLabel });
    }
...
function buildListMessage(entries, diagnostics) {
    ...
    const lines = [HEADER, "", `📋 الطلبات المعلقة (${entries.length}):`, "", diagLine, ""];
    // Group by folder
...
class RequestsPlugin {
            ...
            async execute(ctx) {
                ...
                if (sub === "فحص" || sub === "direct" || sub === "id") {
                    ...
                    try {
                        ...
                        await ctx.reply(`❌ تعذّر القبول المباشر.\n` +
                ...
                if (isAccept || isDelete) {
                    ...
                    if (isNaN(idx) || idx < 1) {
                        await ctx.reply(`⚠️ الرجاء تحديد رقم القروب.\n\n` +
    ...
    async onDisable() { this.ctx.logger.info("RequestsPlugin disabled."); }
    async onUnload() { this.ctx.logger.info("RequestsPlugin unloaded."); }
}
exports.default = new RequestsPlugin();
