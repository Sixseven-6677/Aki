...
const DATA_PATH = path_1.default.resolve("data/black-plugin.json");
function loadStore() {
    try {
 ...
 */
function rawBodyAfterArgs(rawText, n) {
    let s = rawText;
    for (let i = 0; i < n; i++) {
        s = s.replace(/^\s*\S+/, "");
    }
...
class BlackPlugin {
    ...
    async onLoad(ctx) {
        this.ctx = ctx;
        this.repo = ctx.consumeService("black-config-repo") ?? null;
        if (this.repo) {
            try {
    ...
    async saveAll(caller) {
        if (this.repo) {
            const promises = Object.keys(this.store.threads).map((threadId) => this.repo.upsert(threadId, {
    ...
    // ── Timer helpers ─────────────────────────────────────────────────────────
    startTimer(pCtx, threadId) {
        if (this.activeTimers.has(threadId))
            return;
        ...
        const disposable = pCtx.scheduleRecurring({
            ...
            fn: async () => {
                ...
                const sender = pCtx.consumeService("facebook-sender");
                if (!sender) {
                ...
                try {
                    ...
                    if (plugin.repo) {
                        plugin.repo.upsert(threadId, { lastSentAt: new Date() }).catch((err) => {
    ...
    async handleSetMessage(ctx, pCtx) {
        await ctx.typingOn();
    ...
    async handleSetInterval(ctx, pCtx) {
        await ctx.typingOn();
...
exports.default = new BlackPlugin();
