...
exports.CommandPipeline = void 0;
const IMiddleware_1 = require("../middleware/types/IMiddleware");
const LoggerManager_1 = require("../logger/LoggerManager");
const log = LoggerManager_1.LoggerManager.getLogger("CommandPipeline");
class CommandPipeline {
    ...
    use(middleware) {
        ...
        const fn = isObj ? (0, IMiddleware_1.toMiddlewareFn)(middleware) : middleware;
        const name = isObj
            ? middleware.name
    ...
    async run(ctx) {
        ...
        if (currentPrefix) {
            if (!rawName.startsWith(currentPrefix)) {
                log.debug(`[${traceId}] ✗ Prefix "${currentPrefix}" not matched — not a command.`, { rawName });
                return;
            }
    ...
    async runStep(ctx, command, index, traceId, pipelineStart) {
        ...
        log.debug(`[${traceId}] → [${index + 1}/${this.steps.length}] middleware:"${step.name}"`, { userId: ctx.user.id, command: command.name });
        try {
            ...
            const msg = err instanceof Error ? err.message : String(err);
            log.error(`[${traceId}] ✗ middleware:"${step.name}" THREW ERROR ` +
                `after ${stepMs}ms: ${msg}`);
            throw err;
        }
        const stepMs = Date.now() - stepStart;
        if (nextCalled) {
            ...
            log.debug(`[${traceId}] ✗ "${step.name}" STOPPED pipeline | ${stepMs}ms` +
