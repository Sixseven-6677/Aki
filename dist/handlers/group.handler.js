...
exports.setGroupSender = setGroupSender;
exports.setGroupBotUserId = setGroupBotUserId;
exports.setGroupApiGetter = setGroupApiGetter;
...
exports.handleNicknameChanged = handleNicknameChanged;
exports.logProtectionEnabled = logProtectionEnabled;
exports.logProtectionDisabled = logProtectionDisabled;
const env_1 = require("../config/env");
const ProtectionRegistry_1 = require("../protection/ProtectionRegistry");
const LoggerManager_1 = require("../logger/LoggerManager");
const log = LoggerManager_1.LoggerManager.getLogger("GroupHandler");
// ── Singleton references (primary account defaults) ────────────────────────
let _sender;
let _botUserId = "";
let _apiGetter = null;
function setGroupSender(s) { _sender = s; }
function setGroupBotUserId(id) { _botUserId = id; }
function setGroupApiGetter(g) { _apiGetter = g; }
// ── Helpers ────────────────────────────────────────────────────────────────
/** Returns the account-specific sender if provided, otherwise the global primary sender. */
function resolveSender(senderOverride) {
    const s = senderOverride ?? _sender;
    if (!s)
        throw new Error("GroupHandler: sender not wired.");
    return s;
}
function getFcaApi() {
    return _apiGetter?.() ?? null;
}
async function notifyAdminBotAdded(sender, threadId) {
    const adminIds = env_1.config.bot.adminIds;
    if (adminIds.length === 0) {
        log.warn("GroupHandler: bot added to group but BOT_ADMIN_IDS is empty — no notification sent.", { threadId });
        return;
    }
    const msg = `✅ تم إضافة البوت إلى مجموعة جديدة!\n` +
...
// ── Public handlers ────────────────────────────────────────────────────────
async function handleMemberJoined(event, senderOverride) {
...
async function handleMemberLeft(event, _senderOverride) {
    if (event.members.length === 0) {
 ...
 */
async function handleNameChanged(event, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
apiGetterOverride) {
    const store = (0, ProtectionRegistry_1.getProtectionStore)();
    const state = store.threads[event.threadId];
    // ── Log every name_changed event unconditionally ──────────────────────────
    (0, ProtectionRegistry_1.recordNameEvent)(event.threadId);
    ...
    const api = (apiGetterOverride ? apiGetterOverride() : getFcaApi());
    if (!api) {
        log.warn("GroupHandler: name_changed — protection active but FCA API unavailable (mid-reconnect?). [name-api-unavailable]", {
            threadId: event.threadId,
            newName: event.newName,
            lockedName: state.lockedName,
            changedBy: event.changedBy,
    ...
    log.warn("GroupHandler: external name change detected — reverting. [name-revert-start]", {
        threadId: event.threadId,
    ...
    await new Promise((resolve) => {
        api.setTitle(lockedName, event.threadId, (err) => {
            if (err) {
                ...
                (0, ProtectionRegistry_1.recordRevert)(event.threadId);
...
async function handleNicknameChanged(event, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
apiGetterOverride) {
    const store = (0, ProtectionRegistry_1.getProtectionStore)();
    // Use the per-account getter when provided (secondary account protection fix),
    ...
    if (event.participantId === botId) {
        ...
        if (!protectedNick)
            return;
        if (event.newNickname === protectedNick)
            return;
    ...
    const state = store.threads[event.threadId];
    if (!state?.protectNicknames)
        return;
    const expected = state.nicknames[event.participantId];
    if (!expected)
        return;
    if (event.newNickname === expected)
        return;
...
function logProtectionEnabled(threadId, lockedName, by) {
    (0, ProtectionRegistry_1.recordProtectionEnabled)(threadId);
...
function logProtectionDisabled(threadId, by) {
