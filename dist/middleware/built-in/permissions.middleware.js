...
const log = LoggerManager_1.LoggerManager.getLogger("Middleware/Permissions");
function createPermissionsMiddleware(opts) {
    ...
    return {
        ...
        handle: async (ctx, command, next) => {
            ...
            if (command?.adminOnly) {
                ...
                if (!isAdmin) {
                    log.warn(`Permissions: user ${userId} tried admin-only command "${command.name}" — denied.`);
            ...
            if (opts.check) {
                ...
                if (!allowed) {
                    log.warn(`Permissions: user ${userId} failed custom check for "${command?.name ?? "(no-command)"}".`);
                    await ctx.reply(denyMsg);
                    return;
                }
            }
