...
exports.createCooldownMiddleware = createCooldownMiddleware;
const LoggerManager_1 = require("../../logger/LoggerManager");
const log = LoggerManager_1.LoggerManager.getLogger("Middleware/Cooldown");
function createCooldownMiddleware(opts) {
    /**
    ...
    return {
        ...
        handle: async (ctx, command, next) => {
            ...
            if (remaining > 0) {
                ...
                log.debug(`Cooldown active for user ${ctx.user.id} on command "${command.name}" (bot: ${botId}) — ` +
                    `${seconds}s remaining.`);
                const msg = opts.message?.(seconds, command.name) ??
