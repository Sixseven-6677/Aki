...
exports.createAntiSpamMiddleware = createAntiSpamMiddleware;
const LoggerManager_1 = require("../../logger/LoggerManager");
const log = LoggerManager_1.LoggerManager.getLogger("Middleware/AntiSpam");
function createAntiSpamMiddleware(opts) {
    const store = new Map();
    ...
    return {
        ...
        handle: async (ctx, _command, next) => {
            ...
            record.timestamps = record.timestamps.filter((t) => now - t < opts.windowMs);
            if (record.timestamps.length >= opts.maxMessages) {
                ...
                log.warn(`AntiSpam blocked user ${userId} (bot: ${botId}) — ` +
