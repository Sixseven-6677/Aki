...
const log = LoggerManager_1.LoggerManager.getLogger("EmailPasswordProvider");
    ...
    async loginWithCredentials() {
        ...
        return new Promise((resolve) => {
            ...
            const timeout = setTimeout(() => {
                ...
                settle({
                    success: false,
                    failureReason: IAuthPipeline_1.AuthFailureReason.NETWORK_FAILURE,
            ...
            try {
                ...
                fcaLogin({ email: this.email, password: this.password, forceLogin: true }, (err, api) => {
                    clearTimeout(timeout);
                    if (err || !api) {
                        const reason = classifyError(err);
                        const msg = safeErrorMessage(err);
                        log.warn(`EmailPasswordProvider: login failed. ` +
                            `reason="${reason}" error="${msg}"`);
                        settle({ success: false, failureReason: reason, error: msg });
                        return;
                    }
                    try {
                        api.setOptions(FCA_OPTIONS);
                    }
                    catch { /* non-critical */ }
                    const freshCookies = api.getAppState();
                    if (!freshCookies || freshCookies.length === 0) {
                        log.error("EmailPasswordProvider: login reported success but " +
                    ...
                    log.info(`EmailPasswordProvider: login successful. ` +
                    ...
                    settle({ success: true, appState: freshCookies });
                });
            }
            catch (syncErr) {
                clearTimeout(timeout);
...
function safeErrorMessage(err) {
    if (!err)
        return "null error returned by fca-unofficial";
    if (err instanceof Error)
        return err.message;
    if (typeof err === "object") {
        const e = err;
        if (typeof e["errorDescription"] === "string")
...
function classifyError(err) {
    ...
    if (msg.includes("two-factor") || msg.includes("two factor") ||
        msg.includes("verification code") || msg.includes("login approval") ||
        msg.includes("approvals")) {
        return IAuthPipeline_1.AuthFailureReason.TWO_FACTOR_AUTH;
    }
    // Wrong credentials
