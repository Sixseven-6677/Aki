...
const ReconnectGuard_1 = require("./ReconnectGuard");
const SessionHealthMonitor_1 = require("./SessionHealthMonitor");
const IReconnect_1 = require("./types/IReconnect");
const LoggerManager_1 = require("../../logger/LoggerManager");
const log = LoggerManager_1.LoggerManager.getLogger("ReconnectManager");
...
class ReconnectManager {
    ...
    constructor(auth, session, options = {}) {
        ...
        this.policy = new RetryPolicy_1.RetryPolicy(this.opts.retry);
        this.guard = new ReconnectGuard_1.ReconnectGuard({
    ...
    async reconnect(accountId) {
        ...
        if (record?.status === IReconnect_1.ReconnectStatus.CIRCUIT_OPEN) {
            log.warn(`[${accountId}] ⛔ Circuit OPEN — reconnect() blocked. ` +
                `Provide fresh credentials and call resetCircuit("${accountId}") to re-enable. ` +
                `[circuit-open]`, { circuitFailures: this.circuitFailures.get(accountId) ?? 0 });
            return false;
        }
        ...
        if (!this.guard.isAllowed(accountId)) {
            ...
            log.warn(`[${accountId}] Reconnect is spam-guarded` +
                (until ? ` until ${until.toISOString()}.` : "."));
            this.setStatus(accountId, IReconnect_1.ReconnectStatus.BLOCKED);
            return false;
        }
    ...
    resetCircuit(accountId) {
        ...
        this.setStatus(accountId, IReconnect_1.ReconnectStatus.IDLE);
        log.info(`[${accountId}] ✅ Circuit RESET (${prev} → IDLE). ` +
    ...
    summary() {
        ...
        return {
            total: all.length,
            connected: all.filter((r) => r.status === IReconnect_1.ReconnectStatus.CONNECTED).length,
            failed: all.filter((r) => r.status === IReconnect_1.ReconnectStatus.FAILED).length,
            blocked: all.filter((r) => r.status === IReconnect_1.ReconnectStatus.BLOCKED).length,
            circuitOpen: all.filter((r) => r.status === IReconnect_1.ReconnectStatus.CIRCUIT_OPEN).length,
        };
    }
    // ─── Core retry loop ────────────────────────────────────────────────────────
    async runRetryLoop(accountId) {
        this.setStatus(accountId, IReconnect_1.ReconnectStatus.RETRYING);
        const record = this.ensureRecord(accountId);
        let attempt = 0;
        log.info(`[${accountId}] Starting reconnect cycle. Max attempts: ${this.policy.maxAttempts}`);
        while (this.policy.shouldRetry(attempt)) {
            ...
            if (success) {
                ...
                this.setStatus(accountId, IReconnect_1.ReconnectStatus.CONNECTED);
                log.info(`[${accountId}] ✓ Reconnected on attempt ${attempt + 1}. ` +
                    `Circuit closed. [circuit-closed]`);
                return true;
            }
            log.warn(`[${accountId}] ✗ Attempt ${attempt + 1} failed: ${error ?? "unknown"}`);
            attempt++;
        ...
        if (failures >= CIRCUIT_OPEN_AFTER_FAILURES) {
            this.setStatus(accountId, IReconnect_1.ReconnectStatus.CIRCUIT_OPEN);
            log.error(`[${accountId}] ⛔ Circuit OPEN after ${failures} full retry cycle(s). ` +
            ...
            this.setStatus(accountId, IReconnect_1.ReconnectStatus.FAILED);
            log.error(`[${accountId}] ✗ All ${this.policy.maxAttempts} attempts failed. ` +
    ...
    async attemptLogin(accountId) {
        ...
        if (this.auth.hasFallbacks(accountId)) {
            log.info(`[${accountId}] Reconnect: trying email/password fallback first — ` +
                `AppState env cookies are likely expired. [fallback-first]`);
            const fbResult = await this.auth.loginFallbackOnly(accountId);
            if (fbResult.success) {
                log.info(`[${accountId}] Email/password login succeeded — fresh cookies obtained. ` +
                    `[email-password-ok]`);
                sessionRestored = true; // credentials injected — skip restoreSession + auth.login()
            }
            else {
                log.warn(`[${accountId}] Email/password fallback failed: ${fbResult.error ?? "unknown"}. ` +
        ...
        if (this.customCheck) {
            ...
            let connected = false;
            log.info(`[${accountId}] Polling MQTT connectivity (max ${VERIFY_CONNECT_MS / 1000}s)…`);
            ...
            if (!connected) {
                log.warn(`[${accountId}] ✗ MQTT not connected after ${VERIFY_CONNECT_MS / 1000}s. ` +
    ...
    // ─── Health monitor ─────────────────────────────────────────────────────────
    startMonitor() {
        this.monitor = new SessionHealthMonitor_1.SessionHealthMonitor({
            ...
            onDisconnected: (accountId) => {
                ...
                if (record?.status === IReconnect_1.ReconnectStatus.FAILED) {
                    const failures = (this.circuitFailures.get(accountId) ?? 0) + 1;
                    this.circuitFailures.set(accountId, failures);
                    this.setStatus(accountId, IReconnect_1.ReconnectStatus.CIRCUIT_OPEN);
                    log.error(`[${accountId}] ⛔ Health monitor: FAILED → CIRCUIT_OPEN. ` +
                        `Prevented retry loop. [circuit-open]`, { failureCycles: failures });
                    return;
                }
                log.warn(`[${accountId}] Health monitor: disconnected — scheduling reconnect.`);
                this.reconnect(accountId).catch((err) => {
                    log.error(`[${accountId}] Health-monitor reconnect threw.`, err instanceof Error ? err : new Error(String(err)));
                });
            },
    ...
    setStatus(accountId, status) {
        ...
        const emoji = {
            [IReconnect_1.ReconnectStatus.IDLE]: "⚪",
            [IReconnect_1.ReconnectStatus.RETRYING]: "🔄",
            [IReconnect_1.ReconnectStatus.CONNECTED]: "🟢",
            [IReconnect_1.ReconnectStatus.FAILED]: "🔴",
            [IReconnect_1.ReconnectStatus.BLOCKED]: "🚫",
            [IReconnect_1.ReconnectStatus.CIRCUIT_OPEN]: "⛔",
        };
        log.info(`[${accountId}] Status → ${emoji[status]} ${status}`);
    }
    ensureRecord(accountId) {
        if (!this.records.has(accountId)) {
            this.records.set(accountId, {
                accountId,
                status: IReconnect_1.ReconnectStatus.IDLE,
                attempts: [],
                lastAttemptAt: null,
                nextAttemptAt: null,
                blockedUntil: null,
