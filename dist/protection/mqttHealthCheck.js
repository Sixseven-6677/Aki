    ...
    if (cfg.notify) {
        try {
            ...
            if (api)
                for (const id of ids) {
                    try {
                        api.sendMessage(`⚠️ [DAVID V1] إعادة الاتصال التلقائية (#${_count})`, String(id)).catch(() => { });
                    }
                    catch (_) { }
                }
        }
...
function schedule() {
    const cfg = getCfg();
    _t = setTimeout(async () => { try {
        await doCheck();
    }
    catch (_) { } schedule(); }, rand(cfg.minMs, cfg.maxMs));
}
function startHealthCheck() { if (!_t)
    schedule(); }
function stopHealthCheck() { if (_t) {
    clearTimeout(_t);
    _t = null;
} }
